...
export default function Admin() {
    ...
    async function handleUpload(e) {
        ...
        try {
            ...
            const publicUrl = `${r2PublicUrl}/${fullPath}`;
    ...
    return (
        <div className="min-h-screen bg-[#ffcc00] flex items-center justify-center p-6 relative overflow-hidden font-sans">
            ...
            {/* FAIXA GSAP/CSS INFINITA - CENTRALIZADA COMO NA HOME */}
            ...
            {/* Botão Sair - Agora com contraste para o fundo amarelo */}
            <button
            ...
            {/* Painel com estilo Home (Branco com borda marrom e sombra profunda) */}
            <div className="bg-white p-8 rounded-[3rem] shadow-[0_12px_0_0_#4e3629] w-full max-w-md border-4 border-[#4e3629] z-20 relative">
                ...
                <form onSubmit={handleUpload} className="flex flex-col gap-5">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input
                            ...
                            className="w-full p-4 bg-gray-100 border-4 border-transparent focus:border-[#ffcc00] rounded-2xl outline-none font-bold text-[#4e3629] transition-all placeholder:text-gray-300"
                        />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="p-6 border-4 border-dashed border-gray-200 rounded-[2rem] hover:border-[#ffcc00] transition-all flex flex-col items-center gap-2 cursor-pointer relative bg-gray-50/50">
                            ...
                            <p className="text-sm font-black text-gray-400 text-center leading-tight">
                                {file ? file.name : "Toque para escolher a foto"}
                            </p>
                        </div>
                    </div>

                    {/* Botão de Postar - Agora no estilo Red M&M como na Home */}
                    <button
                        disabled={loading}
                        className={`mt-4 p-5 rounded-[2rem] font-[900] text-xl text-white transition-all shadow-[0_8px_0_0_#a0001a] active:shadow-none active:translate-y-1 ${loading ? 'bg-gray-400 shadow-none' : 'bg-[#df0024] hover:bg-[#ff002a]'
                            }`}
                    >
                        {loading ? (
                ...
                {mensagem.text && (
                    <div className={`mt-8 p-5 rounded-2xl flex items-center gap-3 font-bold border-4 animate-in fade-in slide-in-from-bottom-2 duration-500 ${mensagem.type === 'success' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'
                        }`}>
                        {mensagem.type === 'success' ? <CheckCircle2 size={24} /> : <AlertCircle size={24} />}
                        <span className="text-sm uppercase tracking-tight">{mensagem.text}</span>
                    </div>
                )}
            </div>
